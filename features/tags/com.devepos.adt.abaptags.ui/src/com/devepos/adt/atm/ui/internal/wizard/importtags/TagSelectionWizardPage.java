...
public class TagSelectionWizardPage extends AbstractBaseWizardPage {
  ...
  private ProjectInput sourceProjectInput;
  ...
  private IProject previousSourceProject;
  ...
  private void createTargetProjectSection(final Composite parent) {
    targetProjectInput = createProjectInput(parent);
  ...
  private void createSourceProjectControl(final Composite parent) {
    sourceProjectInput = createProjectInput(parent);
    sourceProjectInput.addStatusChangeListener(status -> {
      projectInvalid = !status.isOK();
      if (status.isOK()) {
        var wizard = getWizard();
        final var newProject = sourceProjectInput.getProjectProvider().getProject();
  ...
  private ProjectInput createProjectInput(final Composite parent) {
    var projectInput = new ProjectInput(true);
    ...
    GridLayoutFactory.swtDefaults().numColumns(2).applyTo(projectComposite);
  ...
  private void createFileChooserSection(final Composite parent) {
    ...
    fileInput = new Text(container, SWT.BORDER | SWT.READ_ONLY);
