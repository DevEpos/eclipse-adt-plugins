...
import com.devepos.adt.tools.base.ui.DialogResultPart;
import com.devepos.adt.tools.base.ui.IAdtToolsBaseStrings;
import com.devepos.adt.tools.base.ui.IDialogFilterPart;
...
import com.devepos.adt.tools.base.ui.util.SWTUtil;
import com.devepos.adt.tools.base.ui.viewers.LabelViewer;
...
public abstract class SearchSelectionDialog<R, F> extends TrayDialog {
	...
	protected Control createContents(final Composite parent) {
		...
		updateButtonsEnableState(new Status(IStatus.ERROR, AdtToolsBaseUIPlugin.PLUGIN_ID, EMPTY_STRING));
		...
		this.resultViewPart.addPropertyChangeListener(l -> {
			if (l.getProperty().equals(DialogResultPart.SELECTED_ELEMENTS_PROPERTY)) {
				...
				updateButtonsEnableState(
					selectedElements <= 0 ? new Status(IStatus.ERROR, AdtToolsBaseUIPlugin.PLUGIN_ID, "") : null); //$NON-NLS-1$
			}
		});
	...
	private void applyFilter() {
		...
		if (this.searchJob == null) {
			setSearchStatus(new Status(IStatus.INFO, AdtToolsBaseUIPlugin.PLUGIN_ID,
				AdtToolsBaseUIResources.getString(IAdtToolsBaseStrings.SearchUI_Searching_xmsg)));
	...
	private void handleSearchJobDone() {
		...
		if (resultException != null) {
			...
			if (resultException instanceof CoreException) {
				...
				status = new Status(IStatus.ERROR, AdtToolsBaseUIPlugin.PLUGIN_ID, resultException.getLocalizedMessage());
			}
			...
			if (searchResult != null) {
				...
				if (validResultsCount == 0) {
					setSearchStatus(new Status(IStatus.INFO, AdtToolsBaseUIPlugin.PLUGIN_ID,
						AdtToolsBaseUIResources.getString(IAdtToolsBaseStrings.SearchUI_NoResults_xmsg)));
				} else if (!searchResult.isResultComplete()) {
					setSearchStatus(new Status(IStatus.WARNING, AdtToolsBaseUIPlugin.PLUGIN_ID, AdtToolsBaseUIResources
						.format(IAdtToolsBaseStrings.SearchUI_ResultsExceedMaximum_xmsg, validResultsCount)));
				} else if (validResultsCount == 1) {
					setSearchStatus(new Status(IStatus.INFO, AdtToolsBaseUIPlugin.PLUGIN_ID,
						AdtToolsBaseUIResources.getString(IAdtToolsBaseStrings.SearchUI_OneResult_xmsg)));
				} else {
					setSearchStatus(new Status(IStatus.INFO, AdtToolsBaseUIPlugin.PLUGIN_ID,
						AdtToolsBaseUIResources.format(IAdtToolsBaseStrings.SearchUI_SpecificResults_xmsg, validResultsCount)));
				}
			}
	...
	private void updateDetails(final IStructuredSelection selection) {
		...
		if (selection == null || selection.isEmpty()) {
			...
			if (selection.size() > 1) {
				detailViewer.setInput(MessageFormat.format(Messages.SearchSelectionDialog_ItemsSelected, selection.size()));
			} else {
	...
	private class FilterView implements IDialogFilterPart<String> {
		...
		private void createFilterHeader(final Composite parent) {
			...
			this.headerLabel.setText(
				(SearchSelectionDialog.this.filterLabel != null && SearchSelectionDialog.this.filterLabel.trim().length() > 0)
...
//			createViewMenu(header);
	...
	private class ResultView extends DialogResultPart {
		...
		protected void createContent(final Composite parent) {
			...
			if (this.initialSelections != null) {
				this.tableViewer.setSelection(new StructuredSelection(this.initialSelections));
			}
		...
		private void createViewer(final Composite parent) {
			this.tableViewer = new TableViewer(parent, (this.multi ? SWT.MULTI : SWT.SINGLE) | SWT.BORDER | SWT.V_SCROLL);
	...
	private final class SearchJob extends Job {
		...
		public SearchJob(final F filter) {
			super(AdtToolsBaseUIResources.getString(IAdtToolsBaseStrings.SearchUI_Searching_xmsg));
			setSystem(true);
			this.filter = filter;
		}
