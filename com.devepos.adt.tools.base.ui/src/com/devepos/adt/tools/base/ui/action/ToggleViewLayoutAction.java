...
import com.devepos.adt.tools.base.ui.action.RadioActionGroup.IRadioActionToggleListener;
...
public class ToggleViewLayoutAction extends MenuManager implements IRadioActionToggleListener {
	...
	public ToggleViewLayoutAction(final SashForm sashForm, final Control resizedControl, final IPreferenceStore prefStore,
		final String layoutPref) {
		this(sashForm, resizedControl, prefStore, layoutPref, true, true, true);
	}

	public ToggleViewLayoutAction(final SashForm sashForm, final Control resizedControl, final IPreferenceStore prefStore,
		final String layoutPref, final boolean horizontal, final boolean vertical, final boolean automatic) {
	...
	private RadioActionGroup createActionGroup(final boolean horizontal, final boolean vertical, final boolean automatic) {
		...
		if (horizontal) {
			actionGroup.addAction(ViewLayoutOrientation.HORIZONTAL.name(),
				...
				AdtToolsBaseUIResources.getImageDescriptor(IAdtToolsBaseImages.HORIZONTAL_LAYOUT), false);
		}
		if (vertical) {
			actionGroup.addAction(ViewLayoutOrientation.VERTICAL.name(), Messages.ToggleViewLayoutAction_VerticalOrientation_xmit,
				AdtToolsBaseUIResources.getImageDescriptor(IAdtToolsBaseImages.VERTICAL_LAYOUT), false);
		}
		if (automatic) {
			actionGroup.addAction(ViewLayoutOrientation.AUTOMATIC.name(),
				Messages.ToggleViewLayoutAction_AutomaticOrientation_xmit,
				AdtToolsBaseUIResources.getImageDescriptor(IAdtToolsBaseImages.AUTOMATIC_LAYOUT), false);
		}
		return actionGroup;
	}
