		...
		this.resultViewPart.addPropertyChangeListener(l -> {
			if (l.getProperty().equals(DialogResultPart.SELECTED_ELEMENTS_PROPERTY)) {
				...
				updateButtonsEnableState(
					selectedElements <= 0 ? new Status(IStatus.ERROR, AdtBaseUIPlugin.PLUGIN_ID, "") : null); //$NON-NLS-1$
			}
		});
		...
		if (resultException != null) {
			...
			if (searchResult != null) {
				...
				if (validResultsCount == 0) {
					...
					setSearchStatus(new Status(IStatus.INFO, AdtBaseUIPlugin.PLUGIN_ID,
						AdtBaseUIResources.format(IAdtBaseStrings.SearchUI_SpecificResults_xmsg, validResultsCount)));
				}
			}
