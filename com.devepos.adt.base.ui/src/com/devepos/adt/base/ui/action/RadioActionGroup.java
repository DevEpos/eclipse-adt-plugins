...
public class RadioActionGroup extends ActionGroup {
    ...
    public boolean isActionToggled(final String actionId) {
        final Action action = actions.stream().filter(a -> a.actionId.equals(actionId)).findFirst().orElse(null);
        if (action != null) {
    ...
    private void toggleAction(final String actionId, final boolean setChecked, final boolean notifyListeners) {
        final ToggleAction action = actions.stream().filter(a -> actionId.equals(a.actionId)).findFirst().orElse(null);
        if (action == null) {
            return;
        }
